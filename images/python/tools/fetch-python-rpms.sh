#!/usr/bin/env bash
# Purpose: Fetch the base-python runtime closure from direct CDN pins and verify each blob's sha256, Red Hat
# signature, and locked RPM header fields before installation.
# Role: container-build
# Python-convertible: yes — python lock parsing can converge on images/python/tools/rpmlock.py.
# Relocate: no — copy-and-own build-process fetch script owned by images/python.

set -euo pipefail

usage() {
  cat >&2 << 'EOF'
usage: fetch-python-rpms.sh --targetarch amd64|arm64 --lockfile LOCKFILE --dest DIR
EOF
}
...
case "${targetarch}" in
  ...
  *)
    echo "unsupported TARGETARCH for python RPM fetch: ${targetarch}" >&2
    exit 1
    ;;
esac

[[ -s "${lockfile}" ]] || {
  echo "python RPM lockfile missing or empty: ${lockfile}" >&2
  exit 1
}

mkdir -p "${dest}"
manifest="${dest%/}/direct-python-rpms.lock"
: > "${manifest}"
  ...
  case "${line}" in
    "# direct_rpm: "*)
      ...
      if [[ -n "${direct_extra:-}" || -z "${direct_package}" || -z "${direct_url}" || -z "${direct_sha}" ]]; then
        echo "invalid direct python RPM lock entry: ${line}" >&2
        exit 1
      fi
      [[ "${direct_url}" == https://cdn-ubi.redhat.com/* ]] || {
        echo "direct python RPM source must be the Red Hat UBI CDN for ${direct_package}: ${direct_url}" >&2
        exit 1
      }
      [[ "${direct_sha}" =~ ^[0-9a-f]{64}$ ]] || {
        echo "invalid direct python RPM sha256 for ${direct_package}: ${direct_sha}" >&2
        exit 1
      }
      if [[ -n "${direct_rpm_sha[${direct_package}]+set}" ]]; then
        echo "duplicate direct python RPM lock entry: ${direct_package}" >&2
        exit 1
      fi
...
fetch_one() {
  ...
  [[ -n "${url}" && -n "${expected_sha}" ]] || {
    echo "missing direct python RPM source pin for ${package}" >&2
    exit 1
  }
  [[ "${url##*/}" == "${filename}" ]] || {
    echo "direct python RPM URL filename mismatch for ${package}: expected ${filename}, got ${url##*/}" >&2
    exit 1
  }
  ...
  if [[ "${header_output}" != "${package}|${name}|${epoch}|${version}|${release}|${arch}|${sha256_header}|${sigmd5}" ]]; then
    echo "${path}: RPM header does not match the locked python row" >&2
    exit 1
  fi
...
while IFS='|' read -r package final_rpmdb name epoch version release arch sha256_header sigmd5 extra; do
  ...
  if [[ -n "${extra:-}" ]]; then
    echo "too many columns for python RPM ${package}" >&2
    exit 1
  fi
  for field in "${package}" "${final_rpmdb}" "${name}" "${epoch}" "${version}" "${release}" "${arch}" "${sha256_header}" "${sigmd5}"; do
    [[ -n "${field}" ]] || {
      echo "empty field in python lock row ${package}" >&2
      exit 1
    }
  done
  case "${final_rpmdb}" in
    yes | no) ;;
    *)
      echo "invalid final_rpmdb=${final_rpmdb} for python RPM ${package}" >&2
      exit 1
      ;;
  esac
  case "${arch}" in
    noarch | "${rpm_arch}") ;;
    *)
      echo "locked python package ${package} has wrong arch ${arch} for ${targetarch}" >&2
      exit 1
      ;;
  esac
  if [[ -n "${row_seen[${package}]+set}" ]]; then
    echo "duplicate python package row: ${package}" >&2
    exit 1
  fi
  row_seen["${package}"]=1
...
for direct_package in "${!direct_rpm_sha[@]}"; do
  [[ -n "${direct_rpm_seen[${direct_package}]+set}" ]] || {
    echo "direct python RPM lock entry has no matching package row: ${direct_package}" >&2
    exit 1
  }
done

echo "python RPM direct-CDN fetch verified: ${#row_seen[@]} packages"
