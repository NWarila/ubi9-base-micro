...
# This fork lives at images/python/tools/, three levels below the repository root. Deriving the root
# from the script's own parent (as the root copy does) would resolve images/python and silently pick up
# the ROOT tailoring — a false green, since micro's profile also passes against this image. Resolve
# explicitly, and allow an override for callers that relocate the tree.
repo_root="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}"
if [[ ! -f "${repo_root}/tools/assert-stig-tailoring.py" ]]; then
  echo "repository root resolution failed: ${repo_root} does not contain tools/assert-stig-tailoring.py" >&2
  exit 2
fi
datastream="${STIG_DATASTREAM:-dist/openscap/ssg-rhel9-ds.xml}"
controls="${STIG_CONTROLS:-dist/openscap/stig_rhel9.yml}"
tailoring="${STIG_TAILORING_FILE:-images/python/stig/rhel9-base-python-tailoring.xml}"
justifications="${STIG_JUSTIFICATIONS_FILE:-images/python/stig/tailoring-justifications.json}"
profile="${STIG_PROFILE:-xccdf_org.nwarila.content_profile_ubi9_base_python_stig}"
...
summary="${out_dir}/base-python.${arch}.stig.summary.json"
