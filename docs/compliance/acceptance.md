...
preflight, a pull-request-only release
...
no publisher; that historical limitation is superseded. Current and historical
production evidence is in the
[canonical publication evidence contract](../reference/verification-contract.md#image-family-publication-evidence-contract).
The Python contract does not pin the micro build path and does not make the
Python reducer a claimed merge-blocking context.

Every privileged Python publication job skips on pull requests.
...
| Fixable vulnerability policy | Trivy and Grype independently reject fixable MEDIUM, HIGH, and CRITICAL findings. The only exception is TD-6: `CVE-2026-31790` on exactly `openssl-fips-provider` and `openssl-fips-provider-so` at exactly `3.0.7-8.el9`, retained for the CMVP #4857 module lock. Both native scanner configurations pin that exact CVE/package/version scope, and `tools/verify.py` exact-checks it. | `tools/run-test-gates.sh`, `security/cve-ignore.trivyignore.yaml`, `security/cve-ignore.grype.yaml`, `.github/workflows/publish-image.yaml`, `.github/workflows/python-ci.yaml`, `.github/workflows/publish-python.yaml`, and `tools/verify.py`. |
| Unfixed vulnerability policy | Unfixed vendor findings never block. Trivy and Grype still run over each image and architecture; scanner execution, database freshness, and content-canary failures remain fatal. Complete findings are retained as JSON and SARIF, with distinct code-scanning categories and sealed 90-day raw evidence on publication paths. | `.github/workflows/publish-image.yaml`, `.github/workflows/publish-python.yaml`, `tools/assert-scanner-db-freshness.py`, and `tools/assert-scanner-canary.py`. |
| Absence-proof OpenVEX | Base-python still publishes and verifies the SQLite `component_not_present` and `CVE-2026-53613` `vulnerable_code_not_present` documents. Their claims are enforced independently by build/runtime/SBOM/raw-scanner SQLite checks and by phantom-package checks for `sqlite-libs`, `util-linux`, and `util-linux-core`; the documents do not suppress findings. | `images/python/vex/`, `images/python/tools/assert-raw-scanners-no-sqlite.py`, `tools/assert-no-phantom-packages.py`, `.github/workflows/python-ci.yaml`, `.github/workflows/publish-python.yaml`, and `tools/verify.py`. |
