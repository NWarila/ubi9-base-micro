...
| Base-python publication capability | `main` and `python/v*` pushes in the base repository may push an unaliased candidate by digest. SPDX, CycloneDX, OpenVEX, NIST SP 800-190, and STIG ARF evidence is required and verified on both platform children; the trust contract and SLSA provenance are index-only. A credentialed cache-cold verification completes before aliases. Create-once aliases receive mandatory pre-evidence and pre-apply collision checks plus post-apply readback, but the operation is not atomic against an external writer. A completed publish is distinct from public consumability: only an owner visibility change followed by successful anonymous verification establishes the latter. | `.github/workflows/publish-python.yaml`, `tools/assert-python-alias-policy.py`, `tools/assert-python-attestation.py`, `tools/assert-python-provenance.py`, [`../reference/verification-contract.md`](../reference/verification-contract.md), and [`../TECH-DEBT.md`](../TECH-DEBT.md#td-9-base-python-create-once-alias-external-writer-race). |
...
| Unfixed vulnerability policy | Separately from the fixable gate, every unfixed HIGH or CRITICAL finding from either scanner is default-denied unless a reviewed OpenVEX statement has an accepted clearing status and matches the product. The live `CVE-2026-31790` statement is `affected`; it is disclosure only and clears nothing. The sole `affected` exception is the exact TD-9 accept-and-track path for known-affected `CVE-2026-11940`, exactly `python3.12` and `python3.12-libs` at `3.12.13-3.el9_8.1`, through `review-by 2026-10-01`. For the legacy local-product path, both the in-tool allowlist and canonical reviewed statement must match for one of the two base-python CI products. A second, dormant path requires a digest-addressed child under the pinned `ghcr.io/nwarila/ubi9-base-python` repository and the conjunction of fixed in-tool constraints, version 2 of the canonical statement, and exact digest-verified OCI index evidence supplied by the caller. The closed descriptor policy requires exactly one child for each supported architecture with distinct child digests, only the locked BuildKit attestation shape otherwise, index-wide descriptor-digest uniqueness, and separate child/attestation digest disjointness. That index is a dynamic authorization input and lacks the trusted registry-origin binding required for production; no workflow calls the path and base-python remains externally unpublished. Both paths require byte-canonical raw scanner identities and refuse valid fix evidence. Padded identity evidence is malformed rather than normalized into authorization. This does not make base-python unaffected. | `tools/assert-vex.py`, the CODEOWNERS-gated `vex/` and `images/python/vex/` documents, and `tools/verify.py`. The legacy path is called by `.github/workflows/python-ci.yaml`; no workflow calls the dormant index-child path. |
