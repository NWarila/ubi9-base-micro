...
| Unfixed vulnerability policy | Separately from the fixable gate, every unfixed HIGH or CRITICAL finding from either scanner is default-denied unless a reviewed OpenVEX statement has an accepted clearing status and matches the product. The live `CVE-2026-31790` statement is `affected`; it is disclosure only and clears nothing. The sole `affected` exception is the exact TD-9 accept-and-track path for known-affected `CVE-2026-11940`, exactly `python3.12` and `python3.12-libs` at `3.12.13-3.el9_8.1`, through `review-by 2026-10-01`. For the legacy local-product path, both the in-tool allowlist and canonical reviewed statement must match for one of the two base-python CI products. The production path requires a digest-addressed child under the pinned `ghcr.io/nwarila/ubi9-base-python` repository and the conjunction of fixed in-tool constraints, version 2 of the canonical statement, and exact digest-verified OCI index evidence. The workflow supplies bytes fetched once from the registry by the push-reported digest and corroborated by SHA-256, with the same digest used by every consumer. `assert-vex.py` requires exactly one distinct child per supported architecture, locks the BuildKit attestation platform and annotations, requires descriptor-digest uniqueness, and enforces child/attestation disjointness. It does not constrain attestation cardinality or duplicate references, and it accepts measured `urls`, `data`, or `artifactType` additions on either descriptor kind because it does not close their top-level key sets. It likewise accepts an invented member in a runnable child's `platform` object. Before the VEX gate or any other consumer runs, the stricter publish resolver requires exactly `digest`, `mediaType`, `platform`, and `size` on runnable descriptors, those four keys plus `annotations` on attestation descriptors, exact `architecture` and `os` platform objects, and exactly one attestation reference per child. TD-11 tracks all three measured policy asymmetries. Both product paths require byte-canonical raw scanner identities and refuse valid fix evidence. Padded identity evidence is malformed rather than normalized into authorization. This does not make base-python unaffected. | `tools/assert-vex.py`, `tools/resolve-python-index.py`, the CODEOWNERS-gated `vex/` and `images/python/vex/` documents, and `tools/verify.py`. The legacy path is called by `.github/workflows/python-ci.yaml`; `.github/workflows/publish-python.yaml` is configured to call the published-child path once for each child on a production run. |
