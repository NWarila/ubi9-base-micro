...
| Path | Enforces |
| --- | --- |
| `tools/verify.py` | Repository contract checks include required files, action pins, exact workflow permissions and evidence ordering, Python build/publish workflow controls, native TD-6 scope, SQLite and libuuid absence-proof wiring, signing, provenance, SBOM, NIST/STIG, secret gates, scanner freshness/canaries, and retained Python OpenVEX documents. Static workflow checks are defence-in-depth against accidental regression, not exhaustive interpretation of arbitrary shell or Bake overrides. |
